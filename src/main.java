...
public class main extends JFrame {
     ...
     * */
    private static final String MySQL_DB = "ymgfroht_lpa_ecomms";
    public static final String user      = "ymgfroht_lpa_ecomms";
    public static final String pass      = "5XmvHX4djjzQRMRS"; 
    public static final String url       = "jdbc:mysql://eduaustralia.net:3306/" + MySQL_DB;
//    public static final String url       = "jdbc:mysql://www.eduaustralia.net:2082/" + MySQL_DB; 3306
    ...
    public JTextField txtStockSearch,txtStockID,txtStockName,txtStockDes,
    					txtUsersSearch,txtEditUsername,txtUserID,txtUserFirstName,txtUserLastName,
    					txtSalesSearch,txtClientIDSearch,txtCliendId,txtName,txtPhone,txtItemID,
    					txtNameItem,txtPrice,txtQuantity,txtStockOnHand,txtStockPrice;
...
;			}
	...
	public void openDB() {
        try {
        	...
        	JOptionPane.showMessageDialog(null, "Error1: Impossible to connect to the database!");
		}
	}
			...
			if(validateStockInfo())
			{
				if(saveMode == "new") {
					st.executeUpdate("INSERT INTO lpa_stock "
							...
							+ txtStockID.getText() + "', '"+ txtStockName.getText() +"', '"+ txtStockDes.getText() +"', '"+ txtStockOnHand.getText() +"', '"+ txtStockPrice.getText() +"', 'a' );"); //complete the sql statement, matching database coln names and corresponding values
	...
	private void buildNewInvoicePanel()
	{
		...
		ifNewInvoice.getContentPane().add(lblClienteIdSearch);
		...
		ifNewInvoice.getContentPane().add(txtClientIDSearch);
		...
		ifNewInvoice.getContentPane().add(btnSearchClient);
		...
		ifNewInvoice.getContentPane().add(lblClienteId);
		...
		ifNewInvoice.getContentPane().add(txtCliendId);
		...
		ifNewInvoice.getContentPane().add(txtName);
		...
		ifNewInvoice.getContentPane().add(txtAddress);
		...
		ifNewInvoice.getContentPane().add(txtPhone);
		...
		ifNewInvoice.getContentPane().add(lblPhone);
		
		JLabel lblStockId = new JLabel("Stock ID");
		lblStockId.setBounds(35, 129, 59, 14);
		lblStockId.setFont(new Font("Arial", Font.BOLD, 12));
		lblStockId.setForeground(Color.WHITE);
		ifNewInvoice.getContentPane().add(lblStockId);
		...
		ifNewInvoice.getContentPane().add(txtInvStockID);
		...
		ifNewInvoice.getContentPane().add(btnSearchItem);
		...
		ifNewInvoice.getContentPane().add(lblStockId_1);
		...
		ifNewInvoice.getContentPane().add(txtItemID);
		...
		ifNewInvoice.getContentPane().add(lblNameItem);
		...
		ifNewInvoice.getContentPane().add(txtNameItem);
		...
		ifNewInvoice.getContentPane().add(lblPrice);
		...
		ifNewInvoice.getContentPane().add(txtPrice);
		...
		ifNewInvoice.getContentPane().add(btnAddItem);
		...
		ifNewInvoice.getContentPane().add(newInvoiceScrollPane);
		...
		ifNewInvoice.getContentPane().add(btnSaveInvoice);
		...
		ifNewInvoice.getContentPane().add(btnClose);
	}
	...
	private void searchClientsData(String searchData) {
		try {
			...
			ResultSet rs = (ResultSet) st.executeQuery(
				"SELECT * FROM lpa_clients WHERE lpa_client_status <> 'D' AND " + 
			    "(lpa_client_ID LIKE '%" + searchData + "%' OR " +
			    "lpa_client_firstname LIKE '%" + searchData + "%' OR "+
			    "lpa_client_lastname LIKE '%"+ searchData + "%');" 
			);
			if(rs.next()) {
